...
fromOrdinalDateValid :: Int64 -> Int -> Maybe Day
...
mondayStartWeek date = (fromIntegral ((div d 7) - (div k 7)),fromIntegral (mod d 7) + 1) where
...
sundayStartWeek date =(fromIntegral ((div d 7) - (div k 7)),fromIntegral (mod d 7)) where
	yd = snd (toOrdinalDate date)
...
fromMondayStartWeek :: Int64   -- ^ Year.
                    -> Int     -- ^ Monday-starting week number.
                    -> Int     -- ^ Day of week. 
                               -- Monday is 1, Sunday is 7 (as \"%u\" in 'Data.Time.Format.formatTime').
                    -> Day
fromMondayStartWeek y w d = ModifiedJulianDay (firstDay + yd)
    where yd = firstMonday + 7 * fromIntegral (w-1) + fromIntegral d - 1
...
fromMondayStartWeekValid :: Int64 -- ^ Year.
                    -> Int     -- ^ Monday-starting week number.
                    -> Int     -- ^ Day of week. 
                               -- Monday is 1, Sunday is 7 (as \"%u\" in 'Data.Time.Format.formatTime').
                    -> Maybe Day
fromMondayStartWeekValid year w d = do
	...
	let yd = firstMonday + 7 * fromIntegral (w-1) + fromIntegral d'
